...
- **Backend — ✅ ahora en control de versiones.** Primer commit a `main` en un repo GitHub privado nuevo (nunca había estado en git). Despliegue a producción sigue siendo manual vía `rsync` + rebuild de Docker (no hay `git pull` en el servidor todavía — configurarlo es una mejora pendiente, no urgente). Nota: la carpeta local del proyecto se reorganizó — front y backend ahora viven juntos bajo `alimentia/` en vez de carpetas separadas.
- **Primera ronda de feedback de revisión (BPM) — ✅ ítems pequeños aplicados y desplegados.** Ajustes a Higiene, Equipos, Temperatura, Agua, Materias Primas y No Conformidades (campos nuevos, sin nuevos módulos). Verificado en producción real (login + POST reales, datos de prueba limpiados). **Pendiente:** confirmar con el equipo de BPM los rangos normativos de referencia (cloro, pH, temperaturas) agregados como valores provisionales. Los ítems grandes de esa misma ronda quedan para después: catálogo de materias primas por empresa, alerta por incumplimiento repetido en Higiene, porcentaje + foto en Instalaciones, y la sección INVIMA completa (evaluación de proveedores en el front, liberación de producto, limpieza de áreas con detergente/desinfectante, alertas de plan de muestreo, trazabilidad con parámetros de proceso).
